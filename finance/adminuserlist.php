...
$checksql = "Select * from Users where `ID`!='$email' " . $qq;
...
while ($row = mysqli_fetch_assoc($checkrst)) {
    ...
    $active1 = "";
    $active2 = "";
    if (number_format($rate * intval($row['qty'])) > 0) {
        if (intval($row['step']) > 3) {
            $active1 = "Deactivate";
            $bool = 0;
        } else {
            $bool = 1;
            $active2 = "Resubmit Payment Detail";
            $active1 = "Activate";
        }
    }
    ...
    ?>
                <tr>
                    <td><a
                            href="/UserDetails?id=<?php echo $row['ID']; ?>"><?php echo $row['firstname'] . " " . $row['lastname']; ?></a>
                    </td>
                    ...
                    <td style="    text-align: center;">
                        <?php if ($rowcount3 == 1) {?>
                        <a href="javascript:showMe(<?php echo $uid; ?>)"><img src="/img/document_search.png"
                                style="width: 20px;"></a>
                        <?php }?>
                    </td>
                    <td>
                        <a href="javascript:ActivateUser(<?php echo $uid . "," . $bool; ?>)"
                            id="a<?php echo $uid; ?>"><?php echo $active1; ?></a>
                    ...
                    <td>
                        <a href="javascript:ActivateUser1(<?php echo $uid; ?>)"
                            id="ab<?php echo $uid; ?>"><?php echo $active2; ?></a>
                    </td>
...
function ActivateUser(id, bool1) {
    $("#a" + id).hide();
    $.post("/ActUser", {
            uid: id,
            bool: bool1
