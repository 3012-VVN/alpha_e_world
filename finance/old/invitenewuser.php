...
if (isset($_REQUEST['email']) && $_REQUEST['email'] != "" && isset($_REQUEST['mobile']) && $_REQUEST['mobile'] != "") {
    if ($rowcount >= 5 || $rowcount1 >= 5) {
        ...
        try {
            ...
            $checksql = "INSERT INTO `inviteuser` (`ID`, `userid`, `name`, `email`, `mobile`,`countrycode`,`unique`,`status`) VALUES (NULL, '$email', '" . $_REQUEST['name'] . "', '" . $_REQUEST['email'] . "', '" . $_REQUEST['mobile'] . "','" . $_REQUEST['countrycode'] . "','$uni',0);";
            $checkrst = mysqli_query($con, $checksql);
