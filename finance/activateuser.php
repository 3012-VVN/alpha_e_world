...
if (intval($_REQUEST['bool']) == 1) {
    ...
    $checksql4 = "SELECT * from payourplan WHERE `userid`='$ID'";
    ...
    if ($inactiveuser4 == 0) {
        ...
        if ($val == 4) {
            ...
            for ($i = 1; $i <= 200; $i++) {
                ...
                if ($day == "Sunday" || $day == "Saturday") {
                    ...
                    date('d-m-Y', $int);
                    $inst = "INSERT INTO `payourplan` (`ID`, `userid`, `amount`, `date`,`sdate`, `status`,`planid`,`joinid`) VALUES (NULL, '$ID', '$perday', '$int','" . date('Y-m-d', $int) . "',0,0,'" . $UserInfo1['joinid'] . "');";
        ...
        if ($inactiveuser == 0) {
            ...
            echo $perdayamount = intval($referalog['qty']) / 100;
            echo $amount = $perdayamount * ($bal / 100);

            /*$sql = "INSERT INTO `LevelBonus` (`ID`, `userid`, `fuserid`, `Level`, `Bonus`,`levelno`, `Date`,`sdate`,`status`,`planid`,`joinid`) VALUES (NULL, '$code', '$ID', '$amount', '0',1, now(),$ss,0,$planid,'" . $UserInfo1['joinid'] . "');";
            mysqli_query($con, $sql);*/
            ...
            for ($i = 1; $i <= $days; $i++) {
                ...
                if ($day == "Sunday" || $day == "Saturday") {
                    ...
                    $sql = "INSERT INTO `LevelBonus` (`ID`, `userid`, `fuserid`, `Level`, `Bonus`,`levelno`, `Date`,`sdate`,`status`,`planid`,`joinid`) VALUES (NULL, '$code', '$ID', '$amount', '0',1, '" . date('Y-m-d', $int) . "',$ss,0,$planid,'" . $UserInfo1['joinid'] . "');";
                    mysqli_query($con, $sql);
            ...
            if ($code != 1) {
                ...
                if ($inactiveuser == 0) {
                    ...
                    for ($i = 1; $i <= $days; $i++) {
                        ...
                        if ($day == "Sunday" || $day == "Saturday") {
                            ...
                            $sql = "INSERT INTO `LevelBonus` (`ID`, `userid`, `fuserid`, `Level`, `Bonus`,`levelno`, `Date`,`sdate`,`status`,`planid`,`joinid`) VALUES (NULL, '$code1', '$ID', '$amount1', '0',2, '" . date('Y-m-d', $int) . "',$ss,0,$planid,'" . $UserInfo1['joinid'] . "');";
                            mysqli_query($con, $sql);
                ...
                if ($code1 != 1) {
                    ...
                    $amount2 = $perdayamount * (2 / 100);
                    $checksql2 = "SELECT * FROM `LevelBonus` WHERE `userid`='$code2' and  `fuserid` = '$ID' and `levelno`=3  and `planid`=$planid";
                    $checkrst2 = mysqli_query($con, $checksql2);
                    $inactiveuser = mysqli_num_rows($checkrst2);
                    if ($inactiveuser == 0) {
                        //$sql = "INSERT INTO `LevelBonus` (`ID`, `userid`, `fuserid`, `Level`, `Bonus`,`levelno`, `Date`,`sdate`,`status`,`planid`,`joinid`) VALUES (NULL, '$code2', '$ID', '$amount2', '0',3, now(),$ss,0,$planid,'" . $UserInfo1['joinid'] . "');";
                        //mysqli_query($con, $sql);
                        ...
                        for ($i = 1; $i <= $days; $i++) {
                            ...
                            if ($day == "Sunday" || $day == "Saturday") {
                                ...
                                $sql = "INSERT INTO `LevelBonus` (`ID`, `userid`, `fuserid`, `Level`, `Bonus`,`levelno`, `Date`,`sdate`,`status`,`planid`,`joinid`) VALUES (NULL, '$code2', '$ID', '$amount2', '0',3, '" . date('Y-m-d', $int) . "',$ss,0,$planid,'" . $UserInfo1['joinid'] . "');";
                                mysqli_query($con, $sql);
        ...
        $inactiveuser = mysqli_num_rows($checkrst2);
        if ($inactiveuser == 0) {
        $sql = "INSERT INTO `LevelBonus` (`ID`, `userid`, `fuserid`, `Level`, `Bonus`,`levelno`, `Date`,`sdate`,`status`,`planid`,`joinid`) VALUES (NULL, '$code1', '$ID', '$amount1', '0',2, now(),$ss,0,$planid,'" . $UserInfo1['joinid'] . "');";
        mysqli_query($con, $sql);
        }
        ...
        $inactiveuser = mysqli_num_rows($checkrst2);
        if ($inactiveuser == 0) {
        $sql = "INSERT INTO `LevelBonus` (`ID`, `userid`, `fuserid`, `Level`, `Bonus`,`levelno`, `Date`,`sdate`,`status`,`planid`,`joinid`) VALUES (NULL, '$code2', '$code1', '$amount', '0',4, now(),$ss,0,$planid,'" . $UserInfo1['joinid'] . "');";
        mysqli_query($con, $sql);
        }
        }
        }*/
...
//--------------------------------------
    ...
    $val = 2;
    $msg = "User has been Deactivated" . $perdayamount;
